...
function sel_row(obj) {
    //没有选中直接返回
    //nalan_*:jQuery:checkbox:*;
    if (!$(obj).is(':checked'))//nalan?   obj不加$号，或者加了$不加括号都不行，why？
        return;
    // alert($(obj).parent().text());
    var str = {"userGroupId": $(obj).parent().text()};
    if (str[0] != "" && !STATE_SUBMITING) {
        //nalan_*:jason:js:*;  组装json数据

        // alert(str);
        $.ajax({
                type: 'post',
                url: '/jsp/setup/showUserGroup', /*${pageContext.request.contextPath}/!*/
...
function refillShowUserGroupTable(data){
    if (data == "") return;
    //处理用户表
    ...
    var str1;
    var datetime;
    for (var i = 0; i < data.length; i++) {//这个结构应该有更好的设计
        datetime = new Date(data[i].createDatetime);
        str1 += "<tr><p>";
        str1 += '<td><input class="checkbox_normal" type="checkbox">' + '</input>';
        /* <!--onclick="del_user_row(this)"-->*/
        str1 += data[i].userId + '</td>';
        str1 += '<td id="td_user_2">' + data[i].userName + '</td>';
        str1 += '<td id="td_user_3">' + data[i].userType + '</td>';
...
function refillUser2GroupTable(data) {
    ...
    for (var i = 0; i < data.userGroupList.length; i++) {
        str += "<tr><p>";
        str += '<td><input class="checkbox_normal" type="checkbox" onclick="sel_row(this)">' + '</input>';
...
function refillUserTable(data) {
    ...
    for (var i = 0; i < data.innerUserList.length; i++) {//这个结构应该有更好的设计
        ...
        str += "<tr><p>";
        str += '<td><input class="checkbox_normal" type="checkbox">' + '</input>';
        /* <!--onclick="del_user_row(this)"-->*/
...
/*    $("#btn_test").click()(function () {
    ...
    $("#btn_del_user").click(function (){
        ...
        $.each($("#table_user input:checked").parent(), function (i, dom) {
            userIdList[i] = dom.innerText;
        })
        $.ajax({
            type: 'post', url: '/jsp/setup/delUser',//@RequestBody绝对不能用get
            contentType: 'application/json;charset=utf-8',
            data: JSON.stringify(userIdList),   //将对象变成字符串
            success: function (data) {
                // alert(data.userGroupName);
                STATE_SUBMITING = false;
                if (data == "")//后台返回对象为null时，转为json后传到前台为空
    ...
    $("#btn_add_user2group").click(function () {
        ...
        $.ajax({
            type: 'post', url: '/jsp/setup/addUser2Group',//@RequestBody绝对不能用get
            contentType: 'application/json;charset=utf-8',
    ...
    $("#btn_add_user").click(function () {
        $("#table_user tr:last").after('<tr><p>' + '<td id="td_user_1"><input class="checkbox_normal" type="checkbox" onclick="sel_row(this)"/>测试test</td>' +
...
//                obj.css("background-color","#00A2CA");
    ...
    $("#btn_add").click(function () {//nalan_*:jQuery:table:*;
        $("#table_user_group tr:last").after('<tr><p>' + '<td id="td_1"><input class="checkbox_normal" type="checkbox" onclick="sel_row(this)"/>测试test</td>' +
            '<td id="td_2">test1</td>' +
            '<td id="td_3">test1</td>' +
            '<td id="td_4">test1</td>' +
...
//                obj.css("background-color","#00A2CA");
    ...
    $("#btn_delete").click(function () {
        ...
        $.ajax({
            type: 'post', url: '/jsp/setup/addUserGroup2',//@RequestBody绝对不能用get
            contentType: 'application/json;charset=utf-8',
